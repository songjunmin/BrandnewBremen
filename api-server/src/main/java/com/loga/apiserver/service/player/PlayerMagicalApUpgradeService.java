...
@Service
public class PlayerMagicalApUpgradeService extends PlayerStatUpgradeService {
    private static final int MAGICAL_AP_INCREASE_AMOUNT = 1;
    private static final int CONSUMPTION_GOLD = 5;
    private static final int MAXIMUM_STAT_AMOUNT = 15;
    private final PlayerService playerService;

    @Autowired
    public PlayerMagicalApUpgradeService(PlayerService playerService) {
        super(playerService);
    ...
    public int statUpgrade(Long id) {
        ...
        if(foundPlayer.getMagicalAp() >= MAXIMUM_STAT_AMOUNT) {
            throw new MaxStatAmountExceededException("강화로 올릴 수 있는 최대 체력에 도달했습니다.");
        }
        return super.logic(id, MAGICAL_AP_INCREASE_AMOUNT, CONSUMPTION_GOLD, foundPlayer::increaseMagicalAp).getMagicalAp();
    }
}
